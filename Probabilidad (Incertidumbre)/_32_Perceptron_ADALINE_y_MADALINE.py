...
class Perceptron:  # Define la clase Perceptrón
    def __init__(self, num_inputs, learning_rate=0.05, epochs=200):  # Define el método de inicialización
...
class Madaline:  # Define la clase Madaline
    def __init__(self, num_inputs, num_outputs, learning_rate=0.05, epochs=200):  # Define el método de inicialización
...
X = np.array([[0, 0], [0, 1], [1, 0], [1, 1]])  # Define los datos de entrada X para las operaciones lógicas
y_and = np.array([0, 0, 0, 1])  # Define las etiquetas correspondientes a la operación AND
y_or = np.array([0, 1, 1, 1])  # Define las etiquetas correspondientes a la operación OR
y_xor = np.array([0, 1, 1, 0])  # Define las etiquetas correspondientes a la operación XOR

print("Perceptrón AND:")  # Imprime un mensaje indicando la operación
perceptron_and = Perceptron(num_inputs=2)  # Crea una instancia de Perceptrón para la operación AND
perceptron_and.train(X, y_and)  # Entrena el Perceptrón con los datos de entrada y las etiquetas correspondientes a la operación AND
...
print("\nPerceptrón OR:")  # Imprime un mensaje indicando la operación
perceptron_or = Perceptron(num_inputs=2)  # Crea una instancia de Perceptrón para la operación OR
perceptron_or.train(X, y_or)  # Entrena el Perceptrón con los datos de entrada y las etiquetas correspondientes a la operación OR
...
print("\nADALINE XOR:")  # Imprime un mensaje indicando la operación
adaline_xor = Adaline(num_inputs=2)  # Crea una instancia de Adaline para la operación XOR
adaline_xor.train(X, y_xor)  # Entrena Adaline con los datos de entrada y las etiquetas correspondientes a la operación XOR
...
print("\nMADALINE XOR:")  # Imprime un mensaje indicando la operación
madaline_xor = Madaline(num_inputs=2, num_outputs=1)  # Crea una instancia de Madaline para la operación XOR
madaline_xor.train(X, y_xor)  # Entrena Madaline con los datos de entrada y las etiquetas correspondientes a la operación XOR
